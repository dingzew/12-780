...
function searchInputChange()
{
    drawings = [];
    var HTMLDrawingList = document.getElementById("DrawingIDList");
    var length = HTMLDrawingList.length;
    for (i = 0; i < length; i++) {
        HTMLDrawingList.options[i] = null;
    }
    var sendParameters = document.getElementById("searchInput").value;
    var requestsent = "?request=" + sendParameters;
        ...
        xhttp.onreadystatechange = function()
            {
                if(xhttp.readyState == 4 && xhttp.status == 200)
                {
                    ...
                    else
                    {
                        ...
                        addDrawingInfoToList1();
                        drawingInfoListChange();
                    }
                }
            };
        xhttp.open("GET", "/searchDrawing/"+ requestsent,true);
        xhttp.send();
    }
...
function addDrawingInfoToList1() {
    if (drawings.length == 0) {
        alert("Please load the initial drawings to the array first.");
        return;
    }
